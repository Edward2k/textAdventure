...
| **Problem**            | We have now defined a way of sending commands to the server in a set language. These different commands all have different actions and need to retreive or alter different parts of the code. The problem now is that you would not want to couple one player client request to a single class of the server. Coupling one request to a single class is very inflexible, rewriting the handler of the request needs code changes on both ends and makes it impossible to support multiple receivers. |
